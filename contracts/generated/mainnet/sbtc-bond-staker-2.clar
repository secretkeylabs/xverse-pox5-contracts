...
(define-constant TREASURY .sbtc-bond-treasury-2)
...
(define-read-only (get-treasury-balance)
  (contract-call? .sbtc-bond-treasury-2 get-balance)
)
...
(define-public (stake (manager <signer-manager-trait>))
  (let (
    ...
    (if (> sats custodied)
      (try! (contract-call? .sbtc-bond-treasury-2 payout (- sats custodied)
        current-contract
      ))
      u0
    )
        ...
        (let ((registered (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 register-for-bond
          ...
          (if (< sats custodied)
            (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer (- custodied sats) tx-sender .sbtc-bond-treasury-2 none
            ))
            true
          )
          registered
    ...
    (let ((result (if (> sats u0)
        (try! (as-contract?
          ...
          (let ((unstaked (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 unstake-sbtc
            ...
            (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer sats tx-sender .sbtc-bond-treasury-2 none
            ))
            unstaked
          )))
        {
...
(define-public (sweep-unattributed-principal (recipient principal))
  (let ((amount (get-unattributed-principal)))
    ...
    (try! (authorize-operator))
    (asserts! (> amount u0) ERR_NOTHING_TO_CLAIM)
    (try! (contract-call? .sbtc-bond-treasury-2 payout amount recipient))
    (print {
...
(define-private (queue-for-next-bond
  ...
  (begin
    (if (> sats u0)
      (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
        transfer sats depositor .sbtc-bond-treasury-2 none
      ))
      true
    )
    (if (> ustx u0)
...
(define-private (pay-principal
  ...
  (begin
    (if (> sats u0)
      (try! (contract-call? .sbtc-bond-treasury-2 payout sats recipient))
      u0
    )
    (if (> ustx u0)
