...
;; canonical-source-sha256: bf719d17988e5d5a2d87afcf14b455485b374e0165888d11a0524cc2e6d7db96
;; target-network: mainnet; lane: 1
...
(define-constant LANE_ID u1)
(define-constant LANE_COUNT u6)
(define-constant TREASURY .sbtc-bond-treasury-v1-1)
...
(define-read-only (get-treasury-balance)
  (contract-call? .sbtc-bond-treasury-v1-1 get-balance)
)
...
(define-read-only (is-epoch-settled (epoch uint))
  (match (map-get? epochs (+ epoch u1))
    next (let ((pox-info (unwrap! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 get-pox-info)
        false
      )))
      (>= burn-block-height
        (+
...
(define-public (stake (manager <signer-manager-trait>))
  (let (
    ...
    (if (> sats custodied)
      (try! (contract-call? .sbtc-bond-treasury-v1-1 payout (- sats custodied)
        current-contract
      ))
      u0
    )
        ...
        (let ((registered (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 register-for-bond
          ...
          (if (< sats custodied)
            (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer (- custodied sats) tx-sender .sbtc-bond-treasury-v1-1
              none
            ))
            true
          )
    ...
    (let ((result (if (> sats u0)
        (try! (as-contract?
          ...
          (let ((unstaked (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 unstake-sbtc
            ...
            (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer sats tx-sender .sbtc-bond-treasury-v1-1 none
            ))
            unstaked
          )))
        {
  ...
  (begin
    ...
    (let ((booked (try! (apply-early-unstake tx-sender sats))))
      ...
      (let ((unstaked (try! (as-contract?
          ...
          (let ((removed (try! (contract-call? 'SP000000000000000000002Q6VF78.pox-5 unstake-sbtc
              manager sats
            ))))
            (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer sats tx-sender .sbtc-bond-treasury-v1-1 none
...
(define-public (sweep-unattributed-principal (recipient principal))
  (let ((amount (get-unattributed-principal)))
    ...
    (try! (authorize-operator))
    (asserts! (> amount u0) ERR_NOTHING_TO_CLAIM)
    (try! (contract-call? .sbtc-bond-treasury-v1-1 payout amount recipient))
    (print {
...
(define-private (queue-for-next-bond
  ...
  (begin
    (if (> sats u0)
      (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
        transfer sats depositor .sbtc-bond-treasury-v1-1 none
      ))
      true
    )
    (if (> ustx u0)
...
(define-private (pay-principal
  ...
  (begin
    (if (> sats u0)
      (try! (contract-call? .sbtc-bond-treasury-v1-1 payout sats recipient))
      u0
    )
    (if (> ustx u0)
