...
;;
;; A live member may remove part or all of their bonded sBTC before term. The
;; sats leave PoX-5 immediately and become claimable from the paired treasury;
;; attributed STX stays locked until rollover or normal wind-down. Live shares
;; shrink with principal, while a credit offset preserves reward credit already
;; recognized. Any unconsumed rollover commitment must be cancelled first.
;;
;; Epoch rewards
;;
;; Each opened bond is an epoch with immutable stake metadata, mutable live
;; shares, and a reward accumulator. Rewards arrive as bare sBTC transfers to
;; this contract. A prior epoch remains
...
;; d8406b725b231d899dfad4d92393422559cd0eda (ISC). Early-withdrawal mechanics
;; were independently adapted and hardened from dd2c38693741305a8f6c51503f2baa2c7d5a0678.
...
(define-constant ERR_REWARDS_PENDING (err u135))
...
(define-constant TREASURY .sbtc-bond-treasury-0)
...
;; the reward fields and live `total-shares`, which shrinks with early sBTC
;; withdrawals.
(define-map epochs
  uint
  {
    ...
    ;; been credited so far. `credit-offset` retains credit belonging to shares
    ;; removed by early withdrawal plus funded credit that index granularity
    ;; cannot distribute, so cumulative credit cannot move backwards.
    reward-index: uint,
    credited: uint,
    credit-offset: uint,
  }
...
(define-read-only (get-member-live-rollover-commitment (member principal))
...
(define-read-only (get-treasury-balance)
  (contract-call? .sbtc-bond-treasury-0 get-balance)
)
...
;; Position discovery for early withdrawal. The read takes only the member;
;; clients derive exact amount-specific results from the maximum and raw risk
;; values without repeating a chain read or scaling an already-floored quote.
(define-read-only (get-early-unstake-preview (member principal))
  (let (
      (live-epoch (if (> (var-get epoch-count) u0)
        (some (- (var-get epoch-count) u1))
        none
      ))
      (reward-epoch (get-reward-epoch))
      (live-shares (match (get-live-epoch)
        live (get total-shares live)
        u0
      ))
      (risk-pot (if (and
          (is-eq reward-epoch live-epoch)
          (is-some live-epoch)
        )
        (get-unrecognized-rewards)
        u0
      ))
      (blocked-by-commitment (match (map-get? member-live-commitment member)
        generation (not (default-to false (map-get? consumed-generations generation)))
        false
      ))
    )
    (match (map-get? members member)
      stored (let (
          (record (settle stored member))
          (maximum (get bonded-sats record))
          (full-risk (if (> live-shares u0)
            (/ (* risk-pot maximum) live-shares)
            u0
          ))
        )
        {
          member: member,
          live-epoch: live-epoch,
          reward-epoch: reward-epoch,
          max-withdrawable-sats: maximum,
          bonded-ustx: (get bonded-ustx record),
          banked-rewards: (get pending record),
          risk-reward-pot: risk-pot,
          risk-total-shares: live-shares,
          at-risk-rewards: full-risk,
          full-exit-requires-reward-sync: (and
            (> maximum u0)
            (is-eq maximum live-shares)
            (> risk-pot u0)
          ),
          blocked-by-exit: (is-some (get exit-epoch record)),
          blocked-by-commitment: blocked-by-commitment,
        }
      )
      {
        member: member,
...
(define-public (stake (manager <signer-manager-trait>))
  (let (
    ...
    (if (> sats custodied)
      (try! (contract-call? .sbtc-bond-treasury-0 payout (- sats custodied)
        current-contract
      ))
      u0
    )
        ...
        (let ((registered (try! (contract-call? 'ST000000000000000000002AMW42H.pox-5 register-for-bond
          ...
          (if (< sats custodied)
            (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer (- custodied sats) tx-sender .sbtc-bond-treasury-0 none
            ))
            true
          )
          registered
...
;; Wind the pool down for good. Permissionless once the live bond's twelve
;; cycles have run, which is also when the pooled STX unlocks. If early exits
;; already removed every sat, finalize locally without a zero-amount PoX/token
;; call; the synthetic result preserves the ordinary success shape.
(define-public (unstake-sbtc (manager <signer-manager-trait>))
  (let (
    ...
    (let ((result (if (> sats u0)
        (try! (as-contract?
          ;; PoX-5 returns sBTC to the staker and unlocks pooled STX. Forward
          ;; exactly the returned principal so this contract holds only rewards.
          (
            (with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
            "sbtc-token" sats
          )
          ...
          (let ((unstaked (try! (contract-call? 'ST000000000000000000002AMW42H.pox-5 unstake-sbtc
            ...
            (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
              transfer sats tx-sender .sbtc-bond-treasury-0 none
            ))
            unstaked
          )))
        {
          staker: current-contract,
          signer: (var-get signer-manager),
          bond-index: (get bond-index live),
          amount-withdrawn-sats: u0,
          new-amount-sats: u0,
        }
      )))
      (var-set protocol-transition-active false)
...
;; disables the sitting one. The final enabled operator cannot disable itself,
;; so the set intentionally remains nonempty. This permanent operator
;; continuity and its trust implications are documented in SECURITY.md.
...
(define-public (cancel-exit)
  (let (
    ...
    (try! (assert-no-protocol-transition))
    ;; A full early withdrawal leaves only STX waiting for release. It cannot
    ;; be cancelled into a live STX-only position with no sats or shares.
...
;; Take a positive amount of the effective sender's current sBTC position back
;; before term. The local ledger is updated first for clear pool errors, then
;; PoX-5 removes exactly the same sats and the staker forwards them to its
;; paired treasury. Any downstream failure rolls the entire transaction back.
(define-public (unstake-sbtc-early
  ...
  (begin
    ...
    (let ((booked (try! (apply-early-unstake tx-sender sats))))
      (var-set protocol-transition-active true)
      (let ((unstaked (try! (as-contract?
          (
            (with-ft 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
            "sbtc-token" sats
          )
...
;; Apply the ledger half of early withdrawal. Settlement first banks every
;; recognized current/tail reward. Frozen commitment snapshots are never
;; resized or bypassed: any unconsumed marker, including a dangling one, fails
;; closed until the existing cancellation/recovery call clears it.
(define-private (apply-early-unstake
  ...
  (let (
      (record (settle (unwrap! (map-get? members member) ERR_NOTHING_DEPOSITED) member))
      (live (unwrap! (get-live-epoch) ERR_NOT_STAKED))
      (epoch (if (> (var-get epoch-count) u0)
        (- (var-get epoch-count) u1)
        u0
      ))
      (held (get bonded-sats record))
      (leaving-all (is-eq sats held))
      (shares-left (if (<= sats (get total-shares live))
        (- (get total-shares live) sats)
        u0
      ))
      (reward-epoch (get-reward-epoch))
      (risk-pot (if (is-eq reward-epoch (some epoch))
        (get-unrecognized-rewards)
        u0
      ))
      ;; Keep eager quote arithmetic total for an excessive user input; the
      ;; explicit amount guard below still returns ERR_INVALID_AMOUNT.
      (safe-sats (if (and
          (<= sats held)
      ...
      (at-risk (if (> (get total-shares live) u0)
        (/ (* risk-pot safe-sats) (get total-shares live))
        u0
      ))
    )
    (asserts! (not (var-get finished)) ERR_ALREADY_UNSTAKED)
    ...
    (ok {
      ...
      reward-epoch: reward-epoch,
      sats: sats,
      max-withdrawable-sats: held,
      remaining-sats: (- held sats),
      shares-after: shares-left,
      bonded-ustx: (get bonded-ustx record),
      banked-rewards: (get pending record),
      risk-reward-pot: risk-pot,
      risk-total-shares: (get total-shares live),
...
;; it cannot become principal, a future epoch's reward, or a sweepable balance.
...
(define-private (queue-for-next-bond
  ...
  (begin
    (if (> sats u0)
      (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
        transfer sats depositor .sbtc-bond-treasury-0 none
      ))
      true
    )
    (if (> ustx u0)
...
(define-private (pay-principal
  ...
  (begin
    (if (> sats u0)
      (try! (contract-call? .sbtc-bond-treasury-0 payout sats recipient))
      u0
    )
    (if (> ustx u0)
...
;; Bring a member's record up to date. Accrue an already persisted predecessor
;; tail before epoch advancement can reuse its single storage slot, then accrue
;; again after the fold for any tail the fold just created. Initial queues enter
;; epoch 0. Every later epoch reads the member's explicit commitment for that
;; epoch's frozen generation: committed members carry in full, everyone else
;; leaves in full.
(define-read-only (settle
  ...
  (release-if-finished (accrue-current (accrue-stash (get record
    (fold advance-epoch CATCHUP_STEPS {
      record: (commit-queue (accrue-stash record) member),
      member: member,
      done: false,
    })
  ))))
